...
def p1():
    ...
    str_cities = str(list_cities).replace(' ', '')
    str_cities = str_cities.replace('[', '')
    str_cities = str_cities.replace(']', '')
    str_cities = str_cities.replace("'", '')
    print(f'\nСписок городов:\n{str_cities}')
    ...
    sql = 'SELECT z1,city FROM {} WHERE city = "{}"'.format(tabl_name1, interest_city)
...
преподавателей, а также общее процентное ошношение этих значений."""
    ...
    for i in range(len(data_region)):
        sql = 'SElECT x.region, sum(y.PPS), sum(y.DN) FROM {} x JOIN {} y on x.codvuz = y.codvuz WHERE x.region = "{}"'.format(tabl_name1, tabl_name2, data_region[i][0])
...
Введите номер таблицы для отображения: """)
        if tabl_oper == '1':
            disp_cont(tabl_name1)
        if tabl_oper == '2':
            disp_cont(tabl_name2)
    if num == '2':
        p1()
    if num == '3':
        p2()
